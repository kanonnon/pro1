...
int main()
{
	...
	for (i = 0; i < ny; i++) {
		for (j = 0; j < nx; j++) {
            ...
            outimageR[i][j] = center(array);
			o = 0;
            for (m = -1; m < 2; m++) {
                for (n = -1; n < 2; n++) {
                    if (0 <= i + m && i + m < ny && 0 <= j + n && j + n < nx) {
                        array[o] = inputimageG[i + m][j + n];
                    } else {
                        array[o] = 255;
                    }
                    o++;
            ...
            outimageG[i][j] = center(array);
			o = 0;
            for (m = -1; m < 2; m++) {
                for (n = -1; n < 2; n++) {
                    if (0 <= i + m && i + m < ny && 0 <= j + n && j + n < nx) {
                        array[o] = inputimageB[i + m][j + n];
                    } else {
                        array[o] = 255;
                    }
                    o++;
