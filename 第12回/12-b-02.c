...
int main()
{
	...
	for (i = 0; i < ny; i++) {
		for (j = 0; j < nx * 2; j++) {
            if (j < nx) {
                outimageR[i][j] = inputimage1R[i][j];
			    outimageG[i][j] = inputimage1G[i][j];
			    outimageB[i][j] = inputimage1B[i][j];   
            } else {
                outimageR[i][j] = inputimage2R[i][j - nx];
			    outimageG[i][j] = inputimage2G[i][j - nx];
			    outimageB[i][j] = inputimage2B[i][j - nx];
            }
		}
	}/////////////////////////////////////////////////////////////////////////////
    ...
    im = Generate_BMP(filename1, ny, nx * 2);
	for (i = 0; i < ny; i++) {
		for (j = 0; j < nx * 2; j++) {
