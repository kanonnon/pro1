...
int main()
{
    ...
    printf("input fmin : ");
    scanf("%f", &fmin);
    if (fmax < 0 || 255 < fmax || fmin < 0 || 255 < fmin || fmin > fmax) {
        printf("fmax is greater than fmin and these must be in the range 0 to 255\n");
        for (i = 0; i < ny; i++) {
		    for (j = 0; j < nx; j++) {
        ...
        for (i = 0; i < ny; i++) {
		    for (j = 0; j < nx; j++) {
			    outimageR[i][j] = (inputimageR[i][j] - fmin) / (fmax - fmin) * 255;
			    outimageG[i][j] = (inputimageG[i][j] - fmin) / (fmax - fmin) * 255;
			    outimageB[i][j] = (inputimageB[i][j] - fmin) / (fmax - fmin) * 255;
		    }
	    }
    }
	/////////////////////////////////////////////////////////////////////////////
